...
public class EventService
{
     ...
     */
    public List<EventEntity> getEventsByGroupCode(String sessionToken, String groupCode)
    {
        GroupEntity group = groupService.getGroupByInviteCode(sessionToken, groupCode);
        ...
        return eventRepository.findByGroupCodeOrderByEventTimeAsc(group.getInviteCode());
    }
     ...
     */
    public EventEntity createEvent(String sessionToken, CreateEventRequest request)
    {
        GroupMemberEntity membership = groupService.requireSessionMembership(sessionToken, request.getGroupCode());
        GroupEntity group = membership.getGroup();
        EventEntity entity = new EventEntity();
        entity.setGroup(group);
        entity.setGroupCode(group.getInviteCode());
        entity.setPlayerName(membership.getPlayerName());
     ...
     */
    public List<EventEntity> createEventsFromProgressUpload(String sessionToken, ProgressUploadRequest request)
    {
        return request.getEvents()
            .stream()
            .map(event -> buildProgressEventEntity(sessionToken, request, event))
            .map(eventRepository::save)
            .toList();
    }

    private EventEntity buildProgressEventEntity(String sessionToken, ProgressUploadRequest request, ProgressEventRequest event)
    {
        GroupMemberEntity membership = groupService.requireSessionMembership(sessionToken, request.getGroupCode());
        GroupEntity group = membership.getGroup();
        EventEntity entity = new EventEntity();
